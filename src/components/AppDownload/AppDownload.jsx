...
export default function AppDownload() {
  ...
  useEffect(() => {
    ...
    mm.add('(min-width: 640px) and (max-width: 1023px)', () => {
      ...
      gsap.set(phoneRef.current, { y: 110, opacity: 0, scale: 0.94, force3D: true });
  ...
  return (
    ...
    >
      <div className="container overflow-visible">
        <div
          ref={cardRef}
          className="relative bg-primary-gb rounded-[24px] sm:rounded-[32px] lg:rounded-[36px] px-5 sm:px-8 md:px-12 lg:px-20 py-8 sm:py-10 md:py-14 lg:py-20 shadow-[0_20px_50px_rgba(14,82,255,0.22)] overflow-visible"
        >
          {/* Left Content */}
          <div className="relative z-10 max-w-[340px] sm:max-w-[360px] md:max-w-[430px] lg:max-w-[560px] text-left">
            <h2
              ref={headingRef}
              className="text-2xl sm:text-3xl md:text-[36px] lg:text-[44px] xl:text-[48px] font-black font-heading text-white leading-[1.18] tracking-tight"
            >
              Download <br className="hidden sm:inline" /> Garibook Mobile App
            </h2>
            <p
              ref={subtitleRef}
              className="mt-3 sm:mt-3.5 md:mt-4 lg:mt-5 text-sm sm:text-sm md:text-base lg:text-xl text-white/95 font-normal leading-relaxed max-w-[320px] sm:max-w-[340px] md:max-w-[410px] lg:max-w-[480px]"
            >
              {subtitle || 'Download our Customer, Smart Driver and Enterprise App'}
            </p>
            <div ref={buttonRef} className="mt-5 sm:mt-6 md:mt-8 lg:mt-10">
              <a
                href="https://onelink.to/gbweb"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center justify-center gap-3 bg-warning-gb hover:bg-warning-gb-hover text-dark-gb font-bold text-sm sm:text-sm md:text-base lg:text-lg py-3 sm:py-3.5 md:py-4 px-5 sm:px-6 md:px-8 rounded-2xl shadow-gb-warning hover:shadow-gb-warning-hover hover:-translate-y-0.5 hover:scale-[1.02] active:scale-95 transition-all duration-300 group cursor-pointer w-full sm:w-auto"
              >
          ...
          {/* Right Phone Mockup with Hand: Anchored at bottom right on tablet & desktop */}
          <div
            ref={phoneRef}
            className="mt-8 sm:mt-0 flex justify-center sm:block sm:absolute sm:right-4 md:right-8 lg:right-10 xl:right-20 sm:bottom-0 pointer-events-none z-10"
          >
            <img
              src="/assets/images/app_download_phone.png"
              alt="Garibook Mobile App"
              className="h-[260px] sm:h-[340px] md:h-[390px] lg:h-[470px] xl:h-[500px] w-auto object-contain drop-shadow-[0_20px_35px_rgba(0,0,0,0.2)]"
              loading="lazy"
